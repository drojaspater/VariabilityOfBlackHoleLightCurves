...
def mp_worker(tsnap):
    ...
    i_bghts0 = obsint.brisk_light(
        ...
        np.mod(tsnap + left0, xtend), ## Sumar t0
        np.mod(tsnap + right0, xtend), ## Sumar t0
        xtend
    )
    ...
    i_bghts1 = obsint.brisk_light(
        ...
        interpolated3_R, thetao,
        np.mod(tsnap + left1, xtend), ## Sumar t1
        np.mod(tsnap + right1, xtend), ## Sumar t1
        xtend
    )
    ...
    i_bghts2 = obsint.brisk_light(
        ...
        interpolated3_R, thetao,
        np.mod(tsnap + left2, xtend), ## Sumar t2
        np.mod(tsnap + right2, xtend), ## Sumar t2
        xtend
    )
...
def main():
    ...
    filename=path_bl+"BriskLight%s_dx%s_dt%s_dtM%s_a%s_i%s_%s.csv"%(p_brisk,dx0,dt,dt_movie,spin_case,i_case,i_fname[:-3])
